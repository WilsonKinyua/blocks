...
jQuery(document).ready(function () {
    ...
    if ($("body").hasClass("add_tenant")) {
        $.ajax({
            url: "/api/",
            data: { get_prop: blockId() },
            type: "POST",
            success: function (response) {
                ...
                response.data.forEach(function (prop) {
                    $(".prop_list").append(
                        `<li class="mdl-menu__item prop" data-val="${prop.id}" data-units="${prop.units}" >${prop.name} </li>`
                    );
                });

                getmdlSelect.init(".select-prop");
        ...
        //getmdlSelect.init('.getmdl-select'); componentHandler.upgradeAllRegistered();
    }

    $(".prop_list").on("click", function (e) {
        let units = $(e.target).attr("data-units");

        if ($(prop).text() !== $("#prop").val()) $("#unit").val("");

        $(".units").html("");
        units.split(",").forEach(function (unit) {
            $(".units").append(
                `<li class="mdl-menu__item" data-val="${unit}" >${unit} </li>`
            );
        });

        getmdlSelect.init(".select-unit");
        componentHandler.upgradeAllRegistered();
    });
    ...
    $(".vacate").on("click", function () {
        ...
        $.ajax({
            url: "/api/",
            data: { rm_tenant: blockId(), tenant: $(".vacate").attr("id") },
            type: "POST",
            success: function (data) {
                console.log(data);
                Swal.fire("Tenant Vacated!", "", "success").then(() => {
                    window.location.reload();
                });
            },
            error: function (data) {
    ...
    $(".reminder-btn").on("click", (e) => {
        let tenant = e.target.parentElement.parentElement;
        tenant = JSON.parse($(tenant).attr("data-info"));
        ...
        Swal.fire({
            html: `
                ...
                <div class="card-head"><small class='text-uppercase'> <i class="fa fa-bell"></i> &nbsp; &nbsp;Send Reminders</small></div>
                <div class='row'>
                    <div class="col-lg-12 p-t-20">
                        <div class="mdl-textfield mdl-js-textfield txt-full-width is-upgraded is-dirty" data-upgraded="MaterialTextfield">
                            <textarea class="mdl-textfield__input" rows="4" id="msg-input"  style="outline: none !important;" placeholder="Compose Message:">Hi ${
                                tenant.name.split(" ")[0]
                            }, your rent is now overdue, please make payments to avoid being disconnected.  </textarea>
                            <label class="mdl-textfield__label text-info" for="msg-input">Compose Message:</label>
                            <small>
                                SMS: <a href="#">${tenant.phone}</a>
                                &nbsp;&nbsp;&nbsp;&nbsp;
                                Mail-to: <a href="#">${tenant.email}</a>
                            </small>
                        </div>
                    </div>
                </div>
            ...
            focusConfirm: false,
            confirmButtonText:
                '<i class="fa fa-send fa-rotate-90"></i> Send message ',
            confirmButtonAriaLabel: "Save Changes",
            ...
            if (result.value) {
                tenant.message = $("#msg-input").val();

                $.post("/api/send_sms/", tenant).done(function (data) {
                    console.log(data);
                    Swal.fire(
                        "Sent!",
                        "A reminder has been sent to " + tenant.name + ".",
                        "success"
                    );
                });
            }
    ...
    $("#print_btn").click((e) => {
        ...
        $("#mail_btn, #print_btn").addClass("d-none");

        var body = $("body").html($(".print_section"));

        window.print();

        $("body").html(_body);

        //$('#mail_btn, #print_btn').removeClass('d-none');

        return true;
    });

    //$('#prop').on('change', function(){ getmdlSelect.init('.select-unit'); componentHandler.upgradeAllRegistered(); console.log(' Try upgradeAllRegistered')})
